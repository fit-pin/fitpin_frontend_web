		...
		const commentInfo = async () => {
			try {
				const response = await axios.get(
					`${DATA_URL_APP}api/fit_comment/get_fitcomment`,
					{},
				);
		...
		<div className={styles.App}>
			<header className={styles.header}>
				...
				<div className={styles.right}>
					...
					<span className={styles.bold} onClick={() => navigate('/Fitcomment')}>
						핏코멘트
					</span>
			...
			<div className={styles.content}>
				<div className={styles.leftContent}>
					...
					<p className={styles.description}>당신이 궁금한 옷을 검색해보세요!</p>
				</div>
					...
					<div className={styles.productGrid}>
						{filteredProducts.map((product, index) => (
							<div
								...
								key={index}
								onClick={() =>
									navigate('/Fitcomment2', {
										state: {
											fitStorageKey: product.fitStorageKey,
