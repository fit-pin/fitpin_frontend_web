...
function Service() {
	...
	const fetchInquiries = async (page) => {
		try {
			...
			const inquiriesData = response.data._embedded
				? response.data._embedded.inquiryEntityList
				: [];
			// 각 문의에 대해 댓글 상태 확인
				...
				inquiriesData.map(async (inquiry) => {
					try {
						const commentsResponse = await axios.get(
							`${DATA_URL}inquiry/${inquiry.id}/comments`,
						);
		...
		<div className={styles.App}>
			<header className={styles.header}>
				...
				<div className={styles.right}>
					...
					<span className={styles.bold} onClick={() => navigate('/Service')}>
						고객센터
					</span>
				</div>
			</header>
			<div className={styles.content}>
				<div className={styles.leftContent}>
					...
					<ul className={styles.links}>
						<li>
							<span onClick={() => navigate('/Question')}>자주 묻는 질문</span>
						</li>
						<li>
							<span onClick={() => navigate('/Exchange')}>교환 및 환불</span>
						</li>
					</ul>
				</div>
							...
							<div
								...
								onClick={() => {
									if (inquiry.privacy === 'private') {
										...
										navigate('/Board', {
											state: { id: inquiry.id, page: currentPage },
										});
									}
								}}
							>
								<p className={styles.inquiryDate}>
									{inquiry.createdAt} {inquiry.queryType}
									<span
										className={
											inquiry.hasComments ? styles.inquiryYes : styles.inquiryNo
										}
									>
								...
								<p className={styles.inquiryText}>
									<span style={{ fontWeight: 'bold' }}>
										{inquiry.privacy === 'private' && username !== 'admin'
											? '비공개 게시물'
											: inquiry.subject}
									</span>
